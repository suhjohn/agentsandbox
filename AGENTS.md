...
Compilation of all important info
...
- When behavior changes in any of these files, update that README in the same change:
...
- Keep action names, reducer semantics, drag/drop behavior, and panel placement behavior in sync with code.

# Services API Docs Sync Rule

- Canonical services API surface doc: `agent-manager/src/services/README.md`
...
- Calm and focused: minimal chrome, strong hierarchy, no visual noise fighting with message content.
- Continuous: the user should feel like they are always in one ongoing workspace context, not jumping between disconnected screens.
- Immediate: message send should feel acknowledged instantly (input clears, message appears, response state is obvious).
- Stable: avoid layout jumps, flashing loaders, or state resets that make the interface feel fragile.
- Grounded: the left session list should provide orientation, while the right pane feels like a live working surface.
- Confident: loading and streaming states should feel intentional and informative, never like the app is stuck.
- Quietly powerful: advanced capability is present, but the default interaction remains simple and low-friction.
...
```typescript
// DO: Use precise types for props
...
- **Prefer type inference** for local variables, but annotate function signatures
- **Use `readonly`** for immutable data structures
- **Prefer `interface`** for objects that may be extended, `type` for unions/primitives

## Component Types

```typescript
// DO: Explicit prop types with interface
...
// OR: Function declaration (often preferred)
...
// DO: Use React.MouseEvent, React.KeyboardEvent, etc.
...
```typescript
// DO: Explicitly type useState when type can't be inferred
...
3. **Keep hook order consistent** - React relies on call order
...
// DO: Initialize with function for expensive computations
const [data, setData] = useState(() => {
  return expensiveComputation();
});

// DO: Split unrelated state into separate useState calls
const [name, setName] = useState("");
const [age, setAge] = useState(0);
// NOT: const [state, setState] = useState({ name: '', age: 0 })
```
...
// DON'T: Memoize everything - only when needed for performance
...
## Why Minimize useEffect?

- **Effects run after render** - Causes extra renders and performance overhead
- **Dependency management** - Easy to miss dependencies or create infinite loops
- **Harder to reason about** - Effects create implicit dependencies between renders
- **Brittle code** - Effects that combine many concerns are harder to test and maintain
...
```typescript
// DON'T: Use effect to derive state from props
function NameDisplay({ firstName, lastName }: Props) {
...
// DO: Use useMemo if computation is expensive
function ExpensiveDisplay({ data }: Props) {
...
```typescript
// DON'T: Use effect for user-triggered events
function Button() {
  useEffect(() => {
    // Setting up click handler in effect ❌
  }, [])
}
...
```typescript
// DON'T: Reset state in effect when prop changes
...
// DO: Use key prop to reset component
function App() {
  const [userId, setUserId] = useState('1')
  return <Form key={userId} userId={userId} />
}

// OR: Derive initial state from props
function Form({ userId }: { userId: string }) {
  const [formData, setFormData] = useState(() => {
    return getInitialData(userId)
  })
  // Component remounts when userId changes via key
}
```
...
```typescript
// DO: Subscribe to external data sources
...
```typescript
// DO: Fetch data after mount or when dependencies change
...
// BETTER: Use React Query or SWR for data fetching
...
```typescript
// DO: Direct DOM manipulation when React can't handle it
...
```typescript
// DO: Include all values from component scope
...
```typescript
// DO: Always cleanup subscriptions, timers, listeners
useEffect(() => {
...
// DO: Split into separate effects
useEffect(() => {
  fetchData();
}, []);
...
### 4. Avoid State Updates That Trigger Same Effect

```typescript
// DON'T: Update state that effect depends on
useEffect(() => {
  setCount(count + 1); // ❌ Infinite loop if count is in deps
}, [count]);

// DO: Use functional updates or remove dependency
useEffect(() => {
  setCount((prev) => prev + 1); // ✅ Doesn't need count in deps
}, []);
```

### 5. Extract to Custom Hooks

```typescript
// DO: Extract complex effect logic to custom hook
...
| Mutable values that don't trigger renders | `useRef`                             |
| Complex state logic                       | `useReducer`                         |
| Data fetching                             | React Query, SWR, Apollo             |
| External store subscriptions              | `useSyncExternalStore`               |
| Event handlers                            | Direct event props (`onClick`, etc.) |
...
- ✅ **YES** if you need to:
  - Sync with external systems (APIs, DOM, third-party libraries)
  - Set up subscriptions or event listeners
  - Perform side effects after render (fetching, logging, analytics)
- ❌ **NO** if you can:
  - Compute from props/state during render
  - Handle user events with event handlers
  - Use `useMemo` or `useCallback` for memoization
  - Use `useRef` for mutable values that don't trigger renders

---

# Minimize Function Call Depth and Unnecessary Abstractions

## Why This Matters

- **Readability** - Deep call stacks make code harder to follow and debug
- **Performance** - Each function call has overhead; excessive indirection hurts performance
- **Maintainability** - Over-abstracted code requires jumping through many files to understand behavior
- **Cognitive load** - Developers must hold more context in their heads when tracing through layers

## Principles

### 1. Prefer Inline Logic Over Tiny Functions

```typescript
// DON'T: Create functions for trivial operations
...
### 2. Avoid Wrapper Functions That Add No Value
...
function processClick(e: React.MouseEvent) {
  doAction(e);
}

function doAction(e: React.MouseEvent) {
  // actual logic
}

// DO: Put logic directly where it's needed
function handleButtonClick(e: React.MouseEvent) {
  // actual logic here
}
```

### 3. Flatten Callback Chains

```typescript
// DON'T: Nest callbacks unnecessarily
fetchUser(id)
  .then((user) => processUser(user))
  .then((processed) => formatUser(processed))
  .then((formatted) => displayUser(formatted));

// DO: Combine related operations
fetchUser(id).then((user) => {
  const formatted = formatUserData(user);
  displayUser(formatted);
});

// OR: Use async/await for clarity
...
### 4. Limit Abstraction Layers

```typescript
// DON'T: Create abstraction for the sake of abstraction
...
// DO: Use direct calls when abstraction isn't needed
async function getUser(id: string) {
  return fetch(`/users/${id}`).then((r) => r.json());
}
```

### 5. Colocate Related Logic

```typescript
// DON'T: Split related logic across many files
// utils/validation.ts
...
// user.ts - contains validation, formatting, and processing for users
```

## When Abstraction IS Appropriate

- **Reuse** - Logic is used in 3+ places
- **Complexity** - Function encapsulates genuinely complex behavior
- **Testing** - Abstraction enables easier unit testing
- **Domain boundaries** - Clear separation between different concerns
- **Configuration** - Abstraction allows swapping implementations

## Quick Checklist

Before creating a new function or abstraction:

1. ✅ Is this logic used in multiple places?
2. ✅ Does this function do something non-trivial?
3. ✅ Would inlining this make the code harder to read?
4. ✅ Does this abstraction have a clear, single responsibility?
5. ✅ Can I understand the code flow without jumping to 5+ files?
...
2. ✅ Is this responding to a user event? (Use event handler)
3. ✅ Can I use `useMemo` or `useCallback` instead?
4. ✅ Is this really a side effect that must happen after render?
5. ✅ Are my dependencies correct?
6. ✅ Do I need cleanup?
7. ✅ Should this be extracted to a custom hook?
...
DO NOT WORRY ABOUT BACKWARDS COMPATIBILITY. JUST WRITE CLEAN CODE THAT ACTUALLY FIXES AND IMPROVES THINGS IN A STRAIGHTFORWARD AND MINIMAL WAY.
...
DO NOT WORRY ABOUT LEGACY. DO NOT WORRY ABOUT BACKWARDS COMPATIBILITY. JUST MAKE THE COMPLETE CHANGE.
