...
func TestClientToolRequestRespond(t *testing.T) {
	...
	go func() {
		result, err := manager.Request(context.Background(), "run-1", "user-1", "device-1", "ui_get_state", map[string]any{})
		if err != nil {
...
func TestClientToolRejectsDuplicateAndLateResponses(t *testing.T) {
	...
	go func() {
		_, _ = manager.Request(context.Background(), "run-1", "user-1", "device-1", "ui_get_state", map[string]any{})
	}()
	evt := <-events
	request, _ := evt["request"].(map[string]any)
	requestID, _ := request["requestId"].(string)
