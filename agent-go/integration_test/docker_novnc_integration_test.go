...
func TestDockerNoVNCSmokeAndBasicAPIs(t *testing.T) {
	...
	imageSkillsSymlinkCmd := `AGENT_HOME_PATH="${AGENT_HOME:-/home/agent}"; EXPECTED_IMAGE_SKILLS_DIR="${IMAGE_SKILLS_DIR:-/shared/image/skills}"; test -d "${AGENT_HOME_PATH}/.agents/skills" && test -L "${AGENT_HOME_PATH}/.agents/skills/create-user" && test -f "${AGENT_HOME_PATH}/.agents/skills/create-user/SKILLS.md" && test -f "${AGENT_HOME_PATH}/.agents/skills/create-user/notes.txt" && readlink -f "${AGENT_HOME_PATH}/.agents/skills/create-user" | rg -n "${EXPECTED_IMAGE_SKILLS_DIR}/create-user$" >/dev/null`
