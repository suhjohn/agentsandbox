...
export function CoordinatorAgentShell(props: {
  ...
  const [selectedSessionModel, setSelectedSessionModel] = useState<
    string | undefined
  >(props.initialSessionModel);
  const [
    selectedSessionModelReasoningEffort,
    setSelectedSessionModelReasoningEffort,
  ...
  useEffect(() => {
    if (selectedAgentId.length > 0) {
      const selected = coordinatorAgents.find(
  ...
  const sessionsQuery = useQuery({
    ...
    enabled:
      !!auth.user && !auth.isBootstrapping && selectedAgentId.trim().length > 0,
  });
    ...
    <div className="flex items-center gap-2 border-b border-border px-3 py-2">
      <select
        ...
        onChange={(event) => {
          ...
          const agent = coordinatorAgents.find(
            (current) => current.id === nextAgentId,
          );
          setSelectedAgentId(nextAgentId);
