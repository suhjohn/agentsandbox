...
export async function runModalImageBuild(input: {
  ...
  const runtimeEnvSecret = await createSandboxEnvSecret({
  ...
  const sandbox = await modalClient.sandboxes.create(app, imageForSandbox, {
    ...
    secrets: [runtimeEnvSecret, ...modalSecrets],
...
async function execSandboxTextCommand(
  sandbox: Sandbox,
  command: readonly string[],
  options?: {
    readonly timeoutMs?: number;
    readonly secrets?: readonly Awaited<
...
async function writeSandboxFileIfMissing(
  sandbox: Sandbox,
  path: string,
  contents: string,
): Promise<void> {
...
async function createSandboxEnvSecret(
  values: Record<string, string>,
): Promise<Awaited<ReturnType<typeof modalClient.secrets.fromObject>>> {
  return await modalClient.secrets.fromObject(values);
}

async function snapshotSandboxFilesystem(input: {
  readonly sandbox: Sandbox;
  readonly timeoutMs: number;
  ...
  if (imageId.length === 0) {
    throw new Error(input.missingImageMessage);
  }
  return imageId;
}

function buildTerminalAccess(input: {
  readonly sandboxId: string;
  readonly baseUrl: string;
  readonly auth: SandboxAgentTokenResult;
}): TerminalAccess {
  ...
  return {
    ...
    authTokenExpiresInSeconds: input.auth.expiresInSeconds,
  };
}
...
async function provisionSetupSandboxSshAccess(input: {
  ...
  Pick<SetupSandboxSshAccess, "hostPublicKey" | "hostKeyFingerprint">
> {
...
async function createAgentSandboxModal(input: {
  ...
  try {
    ...
    const runtimeEnvSecret = await createSandboxEnvSecret(sandboxEnv);
    const apiKeySecret =
...
export async function snapshotAgentSandbox(input: {
  ...
  try {
    const imageId = await snapshotSandboxFilesystem({
      sandbox,
...
export async function createSetupSandbox(input: {
  ...
  try {
    ...
    const runtimeEnvSecret = await createSandboxEnvSecret(setupRuntimeEnv);
